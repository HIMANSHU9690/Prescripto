...
const Appointment = () => {
  ...
  const getAvailableSlots = async () => {
    setDocSlots([]); // Reset slots initially

    if (!docInfo || !docInfo.slots_booked) {
      console.warn("Doctor data or slots_booked is not available");
      return; // Exit the function if docInfo or slots_booked is undefined
    }
    ...
    for (let i = 0; i < 7; i++) {
      ...
      while (currentDate < endTime) {
        ...
        // Checking availability in slots_booked
        let day = currentDate.getDate();
        let month = currentDate.getMonth() + 1;
        let year = currentDate.getFullYear();
  ...
  useEffect(() => {
    // Only call getAvailableSlots if docInfo and docInfo.slots_booked are defined
    if (docInfo && docInfo.slots_booked) {
      getAvailableSlots();
    }
  }, [docInfo]);
