...
//     {
//       state==="Sign Up"
//       ? <p> Aready have an account? <span onClick={()=>setState('Login')} className='text-primary underline cursor-pointer'> Login here</span></p>
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      if (data.success) {
        localStorage.setItem('token', data.token);
        setToken(data.token);
        toast.success('Successfully logged in/registered');
      } else {
        toast.error(data.message);
      }
    } catch (error) {
